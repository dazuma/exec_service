...
describe ExecService do
  include ::ExecService::TestHelper

  let(:logger_stringio) { ::StringIO.new }
  let(:logger) { ::Logger.new(logger_stringio) }
  let(:exec) { ::ExecService.new(logger: logger) }
  let(:gem_base_dir) { ::File.dirname(__dir__) }
  let(:tmp_dir) { ::File.join(gem_base_dir, "tmp") }
  let(:input_path) { ::File.join(gem_base_dir, "test-data", "input.txt") }
  let(:output_path) { ::File.join(tmp_dir, "output.txt") }
  let(:output2_path) { ::File.join(tmp_dir, "output2.txt") }
  let(:simple_exec_timeout) { 5 }
  let(:ruby_exec_timeout) { jruby? || truffleruby? ? 10 : simple_exec_timeout }
  ...
  describe "result object" do
    ...
    it "detects nonzero exit codes" do
      skip "https://github.com/oracle/truffleruby/issues/2568" if truffleruby?
      ::Timeout.timeout(simple_exec_timeout) do
    ...
    it "detects EACCES/ENOEXEC" do
      skip "https://github.com/truffleruby/truffleruby/issues/4176" if truffleruby?
      ::Timeout.timeout(simple_exec_timeout) do
        nonexecutable = File.join(gem_base_dir, "README.md")
    ...
    it "detects termination due to signal" do
      skip "Skipped test on Windows" if windows?
      ::Timeout.timeout(simple_exec_timeout) do
  ...
  describe "command form" do
    ...
    it "recognizes an array with array binary" do
      skip "Skipped test on Windows and/or JRuby" if jruby? || windows?
      ::Timeout.timeout(simple_exec_timeout) do
    ...
    it "forks procs" do
      skip "Skipped test because fork is not available" unless allow_fork?
      ::Timeout.timeout(simple_exec_timeout) do
        func = proc do
